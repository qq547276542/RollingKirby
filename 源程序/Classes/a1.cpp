...
bool HelloWorld::init()
{
	...
	CCUserDefault::sharedUserDefault()->setIntegerForKey("scene_num",1);
...
#define MUSIC_FILE "backmusic.mp3"
	...
	addNewenemy(ccp(180,130));
	addNewstar(ccp(320,330));
	addNewstar(ccp(430,310));
	addNewstar(ccp(540,290));
	addNewfinish(ccp(80,110));
	
	cocos2d::CCTMXTiledMap *_tileMap; 
	_tileMap=CCTMXTiledMap::create("a1.tmx");  
...
void HelloWorld::update( float dt )//注意，之前2dx的版本是用ccTime的，在2.0.3版本中，用float类型
{
	...
	CCSprite *zj=(CCSprite*)getChildByTag(7);
